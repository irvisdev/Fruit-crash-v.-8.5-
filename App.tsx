...
import { 
  ...
  ChevronRight, Award, Volume2, VolumeX, Coins, Home,
  ShoppingBag, Plus, Clock, Lock,
  Crown, Flame, Save, Settings, Music, Lightbulb,
  Trees, Cloud, Mountain, Snowflake, Candy, User, Gift, MapPin, Globe, Shuffle
} from 'lucide-react';
...
const App: React.FC = () => {
    ...
    try {
      ...
      if (saved) {
        ...
        return {
          ...
          objective: "Загрузка...",
          storySegment: "Дед Мороз паркует сани...",
          targetScore: 1000,
          targetFruits: [],
  ...
  const isSaleActive = useMemo(() => {
    ...
    const end = new Date(2026, 0, 14, 23, 59, 59);
  ...
  const isLevelComplete = useMemo(() => {
     return gameState.score >= gameState.targetScore && gameState.targetFruits.every(f => f.current >= f.target);
  }, [gameState.score, gameState.targetScore, gameState.targetFruits]);
  ...
  const renderScreen = () => {
                ...
                <div className="p-6 animate-scale-up overflow-y-auto pb-20">
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-lg mb-4 flex items-center justify-between">
                         <div className="flex items-center gap-3 text-purple-600">
                            <Globe size={28} />
                            <h3 className="game-font text-xl ui-text-shadow">{t('language')}</h3>
                         </div>
                         <div className="flex bg-slate-200 rounded-full p-1">
                             <button 
                                 onClick={() => setGameState(prev => ({...prev, settings: {...prev.settings, language: 'ru'}}))}
                                 className={`px-4 py-1 rounded-full font-bold transition-all ${gameState.settings.language !== 'en' ? 'bg-white shadow text-blue-600' : 'text-slate-500'}`}
                             >RU</button>
                             <button 
      ...
      if (gameState.screen === 'loading') {
        return (
          <div className="fixed inset-0 flex flex-col items-center justify-center p-6 text-white z-[200]">
            {GlobalSnow}
            <div className="w-24 h-24 bg-white/20 rounded-full flex items-center justify-center text-5xl animate-bounce shadow-xl">🎁</div>
          ...
          return (
            <div onClick={unlockAudio} className="fixed inset-0 bg-blue-50 flex flex-col text-slate-800 z-[60]" style={safeAreaStyle}>
                <div className="w-full bg-white p-4 pt-8 flex items-center justify-between shadow-sm z-10 animate-scale-up">
                    ...
                    <div className="bg-yellow-400 text-yellow-900 px-3 py-1 rounded-full font-bold flex items-center gap-1 shadow-sm">
                       <Coins size={16} /> {totalCoins}
                    </div>
                </div>
                        ...
                        return (
                            <div key={item.id} className="bg-white p-4 rounded-3xl shadow-lg border-b-4 border-slate-200 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className="text-4xl bg-blue-100 p-3 rounded-2xl relative">
                                        {item.icon}
                                        {isSaleActive && (
          ...
          <div onClick={unlockAudio} className="fixed inset-0 z-[40]">
             ...
             <div className="map-wrapper">
                 ...
                 <div className="absolute top-0 left-0 w-full p-4 z-[100] flex justify-between items-center pointer-events-none animate-scale-up" style={{top: 'max(0px, env(safe-area-inset-top))'}}>
                     ...
                     <div className="pointer-events-auto bg-yellow-400 text-yellow-900 px-4 py-2 rounded-full font-bold flex items-center gap-2 shadow-lg border-2 border-white btn-press">
                        <Coins size={20} /> {totalCoins}
                     </div>
                 </div>
                                ...
                                >
                                    ...
                                    {isCurrent && (
                                        <div className="absolute -top-12 left-1/2 -translate-x-1/2 flex flex-col items-center">
                                            <div className="text-5xl filter drop-shadow-xl animate-bounce z-20">🎅</div>
                                        </div>
                                    )}
                                </div>
                             );
          ...
          <div onClick={unlockAudio} className="fixed inset-0 bg-slate-900 flex flex-col text-white z-[60]" style={safeAreaStyle}>
            <div className="w-full bg-slate-900 border-b border-slate-800 p-4 pt-8 flex items-center justify-between shadow-xl animate-scale-up">
              ...
              <div className="flex gap-4 items-center">
                ...
                <div className="bg-yellow-500/20 px-3 py-1 rounded-full flex items-center gap-1 text-yellow-400 font-bold border border-yellow-500/50">
                   <Coins size={16} /> {totalCoins}
                </div>
              </div>
              <div className="w-8"></div>
        ...
        return (
          <div onClick={unlockAudio} className="fixed inset-0 bg-blue-100 flex items-center justify-center p-6 text-center z-[50]">
            <div className="story-card max-w-xs w-full p-8 rounded-[2rem] border-4 border-blue-300 shadow-2xl animate-in fade-in duration-300 relative bg-white">
              ...
              <h2 className="game-font text-xl text-blue-600 mb-4 mt-4 ui-text-shadow">{t('level')} {gameState.level}</h2>
              <div className="mb-2">
      ...
      return (
        <div onClick={unlockAudio} className={`fixed inset-0 flex flex-col items-center p-4 overflow-hidden select-none z-10 touch-none`} style={{paddingTop: 'max(1rem, env(safe-area-inset-top))', paddingBottom: 'max(1rem, env(safe-area-inset-bottom))'}}>
          ...
          <div className="absolute inset-0 z-0 opacity-40 pointer-events-none" style={{ backgroundImage: `radial-gradient(#94a3b8 10%, transparent 11%)`, backgroundSize: '30px 30px' }}></div>
          <div className="absolute bottom-0 w-full h-32 bg-gradient-to-t from-blue-100 to-transparent z-0 pointer-events-none"></div>
                 ...
                 <div className="flex items-center justify-between mt-1">
                     <div className="flex items-center gap-2">
                         <div className="bg-purple-600 px-3 py-1.5 rounded-xl shadow-inner flex items-center gap-2 text-white">
                             <span className="text-[10px] font-bold opacity-70">{t('moves')}</span>
          ...
          <div className="w-full max-w-md flex items-center gap-2 mb-2 px-1 z-10">
             <div className="w-10 h-10 bg-white rounded-xl border-2 border-blue-200 flex items-center justify-center text-xl shadow-md">🎅</div>
             <div className="flex-1 bg-white/80 p-2 rounded-xl rounded-tl-none shadow-md border border-white">
               {isShuffling ? 
                 <p className="text-slate-800 font-bold italic text-[10px] leading-tight animate-pulse text-red-500">Нет ходов! Перемешиваем...</p> 
                 : 
                 <p className="text-slate-800 font-bold italic text-[10px] leading-tight line-clamp-2">{aiCommentary || gameState.objective}</p>
               }
             </div>
          </div>
                             ...
                             {SHOP_ITEMS.map(item => {
                                 ...
                                 <button key={item.id} disabled={gameState.inventory[item.id as keyof Inventory] === 0}
                                    ...
                                    className={`w-full p-3 rounded-xl flex items-center justify-between border-2 
                                     ...
                                     <span className="flex items-center gap-2 text-xl">{item.icon} <span className="text-sm font-bold">{lang === 'en' ? item.title : (item as any).titleRu}</span></span>
            ...
            {(gameState.moves <= 0 || isLevelComplete) && (
              <div className="absolute inset-0 z-[100] bg-white/95 backdrop-blur-md rounded-[2rem] flex flex-col items-center justify-center p-6 animate-in zoom-in">
                ...
                <div className="flex flex-col items-center mb-6">
                    ...
                    {isLevelComplete && <div className="bg-yellow-100 text-yellow-800 px-4 py-2 rounded-full mt-2 font-bold flex items-center gap-2 border border-yellow-300"><Coins size={18} /> +{gameState.lastLevelReward}</div>}
                </div>
